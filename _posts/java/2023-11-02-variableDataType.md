...
1. 카멜 표기법(camelCase)
     ...
     - 맨 첫 글자를 제외한 합성어의 첫 글자만 대문자로 표기하는 방식
     - 변수, 메소드명 표기시 주로 사용
2. 파스칼 표기법(PascalCase)

     - 각 문자의 첫 글자를 대문자로 표기하는 방식

     - 클래스명 표기시 주로 사용

3. 스네이크 표기법(snake_case)

   - 문자의 띄어쓰기를 언더바(_)로 표기하는 방식
   - 데이터베이스에서 주로 사용
### <br>변수의 선언과 초기화
...
- 자주 사용하는 메소드
  - equals, indexOf, replace, substring 메소드는 String 메소드와 동일한 방법으로 사용
...
- List와 다르게 순서대로 저장되지 않으므로 key값을 이용하여 데이터에 접근한다.
...
- 기본형 변수 중 boolean을 제외한 나머지 변수는 형 변환이 가능하다.


- 표현 범위가 좁은 범위에서 넓은 범위로 저장될 때, 자동 형 변환이 된다.
